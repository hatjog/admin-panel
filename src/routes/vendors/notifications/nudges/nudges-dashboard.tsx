...
export function NudgesDashboardPage() {
  ...
  const [forceDecisionVendor, setForceDecisionVendor] = useState<string | null>(
    null,
  )
  ...
  const dryRunMutation = useMutation<TriggerResponse, Error, CadenceStep>({
    ...
    onSuccess: (res) => {
      toast.success(
        `${STEP_LABELS[res.step]} dry-run: ${res.triggered} eligible`,
      )
    },
    ...
    <div className="flex flex-col gap-6 p-6">
      <Container>
        <div className="px-6 py-4">
          ...
          <Text className="text-ui-fg-subtle mt-1">
            Escalating reminder emails (T-21 → T-14 → T-7 → T-3) for vendors
            who have not yet picked opt-in / opt-out. Audit log dedup —
            already-notified vendors at the current step are skipped.
          </Text>
        </div>
        ...
        <div className="px-6 pb-4">
          ...
          {dashboard && (
            <div className="flex flex-col gap-2">
              <Text>
                <strong>Days to flag flip:</strong>{" "}
                {pending?.days_to_flag_flip ?? "—"}
              </Text>
              <Text>
                <strong>Completion rate:</strong>{" "}
                {completion ? `${(completion.completion_rate * 100).toFixed(0)}%` : "—"}{" "}
                ({completion?.opted_in ?? 0} opt-in / {completion?.opted_out ?? 0}{" "}
                opt-out / {completion?.open ?? 0} open)
              </Text>
            </div>
          )}
        </div>
      ...
      <Container>
          ...
          {STEPS.map((step) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="bg-ui-bg-base h-3 w-full rounded-sm border"
                  aria-label={`${STEP_LABELS[step]} progress`}
                >
                  <div
                    className="bg-ui-tag-blue-bg h-full rounded-sm"
        ...
        {activeStep && (
          <div className="border-ui-border-base bg-ui-bg-subtle mx-6 mb-4 rounded border p-4">
            ...
            <Text className="mt-2">
              About to dispatch nudge {STEP_LABELS[activeStep]} to all eligible
              vendors. Already-notified at this step are skipped via audit log
              dedup. Action is irreversible.
            </Text>
                  ...
                  {dispatches.map((e) => (
                    <tr key={e.id} className="border-ui-border-base border-b text-sm">
                      ...
                      <td className="px-3 py-2">
                        {e.vendor_handle ?? e.vendor_id.slice(0, 8)}
                      </td>
      ...
      <Container>
        <div className="px-6 py-4">
          ...
          <Text className="text-ui-fg-subtle mt-1">
            Last-resort admin intervention for unreachable vendors. Marks
            decision_status without sending email; writes audit entry
            `admin_force_decision`. Vendor self-service opt-in form remains
            the primary path (Story 7.2.1).
          </Text>
        </div>
        <div className="px-6 pb-4">
          <Button
            variant="secondary"
            size="small"
            onClick={() => setForceDecisionVendor("")}
            data-testid="nudge-force-decision-open"
          >
            Open force-decision modal
          </Button>
          {forceDecisionVendor !== null && (
            <div className="border-ui-border-base bg-ui-bg-subtle mt-3 rounded border p-4">
              <Heading level="h3">Force decision for vendor</Heading>
              <Text className="mt-2">
                Action persists `decision_status` directly + audit log entry
                with current admin actor id. Vendor receives no email.
              </Text>
              <input
                type="text"
              ...
              <div className="mt-3 flex gap-2">
                <Button
                  ...
                  onClick={() => {
                    toast.warning(
                      "Force-decision endpoint stub — production wiring is Story 7.2 follow-up.",
                    )
                ...
                <Button
                  variant="secondary"
                  size="small"
                  onClick={() => setForceDecisionVendor(null)}
                >
                  Cancel
                </Button>
              </div>
