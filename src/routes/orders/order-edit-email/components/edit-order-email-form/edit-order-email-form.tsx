...
export function EditOrderEmailForm({ order }: EditOrderEmailFormProps) {
  ...
  const handleSubmit = form.handleSubmit(async data => {
    try {
      ...
      toast.success(t('orders.edit.email.requestSuccess'));
      handleSuccess();
    } catch (error) {
    ...
    >
      ...
      >
        <RouteDrawer.Body
          ...
          data-testid="order-edit-email-body"
        >
          <Form.Field
                ...
                <Form.Item data-testid="order-edit-email-item">
                  ...
                  <Form.Control data-testid="order-edit-email-control">
                    <Input
                      type="email"
                      {...field}
                      data-testid="order-edit-email-input"
                    />
                  </Form.Control>
          ...
          <div className="flex items-center justify-end gap-x-2" data-testid="order-edit-email-form-footer-actions">
            <RouteDrawer.Close asChild>
              <Button
                variant="secondary"
                size="small"
                data-testid="order-edit-email-cancel-button"
              >
                {t('actions.cancel')}
              </Button>
            </RouteDrawer.Close>
