        ...
        <Table data-testid="request-product-category-list-table">
          ...
          <Table.Body data-testid="request-product-category-list-table-body">
            {requests?.map((request, index) => {
              ...
              return (
                <Table.Row key={request.id} data-testid={`request-product-category-list-table-row-${index}`}>
                  <Table.Cell data-testid={`request-product-category-list-table-row-${index}-name`}>{requestData.name}</Table.Cell>
                  <Table.Cell data-testid={`request-product-category-list-table-row-${index}-handle`}>{requestData.handle}</Table.Cell>
                  <Table.Cell data-testid={`request-product-category-list-table-row-${index}-submitted-by`}>{request.seller?.name}</Table.Cell>
                  <Table.Cell data-testid={`request-product-category-list-table-row-${index}-date`}>
                    <div className="flex items-center gap-2">
                      <History />
                      {formatDate(request.created_at!)}
                    </div>
                  </Table.Cell>
                  <Table.Cell data-testid={`request-product-category-list-table-row-${index}-status`}>
                    {getRequestStatusBadge(request.status!)}
                  </Table.Cell>
                  <Table.Cell data-testid={`request-product-category-list-table-row-${index}-actions`}>
                    <RequestMenu
