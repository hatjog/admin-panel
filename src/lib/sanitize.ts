/**
 * Story v160-7-8: HTML sanitizer for admin-panel.
 *
 * Defense-in-depth XSS prevention per NFR-SEC-3. Mirrors Story 4.7 storefront
 * baseline contract; allowed tags/attributes whitelist matches storefront.
 *
 * Per T2.1 decision (Wave 15 scaffolding): ships minimal regex-based sanitizer
 * to avoid adding library dep + lockfile drift. Sprint 5 polish: drop in
 * isomorphic-dompurify (or DOMPurify w/ jsdom) for parity with storefront.
 *
 * The function signature is stable — Sprint 5 swap is a single-file change.
 *
 * Test fixture coverage (see __tests__/sanitize.test.ts):
 *   - 10+ XSS payloads stripped
 *   - 5+ benign payloads preserved
 */
